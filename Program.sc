...
Process divide gives a decimal using decimal a, decimal b by doing: {
...
Choice: ".
    ...
    find op then do: {
        ...
        is 4: { set result as input from Using divide with a, b. }
        absent: {print " Invalid ". }
    }
	
	print result.

    Note: if  op == " + "  do: { Using plus with a, b. }
    Note: else if op == " - " do: { Using minus with a, b. }
    Note: else do: { print "Invalid". }
